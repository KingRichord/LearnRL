...
class PPO(nn.Module):
    ...
    def pi(self, x, softmax_dim = 0):
        x = F.relu(self.fc1(x))       # 观察面粉状态（输入状态x）
        x = self.fc_pi(x)             # 想尝试的摊法（输出原始动作logits）
    ...
    def v(self, x):
        x = F.relu(self.fc1(x))   # 同样观察面粉状态
        v = self.fc_v(x)          # 预测这锅能卖多少钱（状态价值）
        return v
    ...
    def train_net(self):
        ...
        for i in range(K_epoch):
            ...
            """
            td_target = r + gamma * self.v(s_prime) * done_mask  # 这锅实际赚的钱 + 下锅预期
